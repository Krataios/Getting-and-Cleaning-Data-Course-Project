...
#Now we load the packages 
library(dplyr)
library(tidyr)#Firt we downloading the dataset from the link 
if(!file.exists("./data")){dir.create("./data")}
fileUrl <- "https://d396qusza40orc.cloudfront.net/getdata%2Fprojectfiles%2FUCI%20HAR%20Dataset.zip"
download.file(fileUrl,destfile="./data/Dataset.zip")
...
meanstdvalue <- (grepl("activityId" , colNames) | 
...
measuresvalue <- sameset[ , meanstd == TRUE]


# AS a second  the set for Activity 

Actnamevalue <- merge(measures, activityLabels,
                      by='activityId',
                      all.x=TRUE)
